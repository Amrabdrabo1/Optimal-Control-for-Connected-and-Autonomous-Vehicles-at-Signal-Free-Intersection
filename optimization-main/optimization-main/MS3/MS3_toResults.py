...
class SimulatedAnnealing:
    def __init__(self, problem, max_iter=250, init_temp=100, final_temp=1e-03, iter_per_temp=5, cooling_schedule="linear", beta=5, alpha=0.9):
    ...
    def perform_algorithm(self):
        ...
        for iter in range(self.max_iter):
            for _ in range(self.iter_per_temp):
                ...
                fvalue_neighbour = self.problem.cost_func(sol_neighbour)  # f(new u)
...
for i in range(10):
    print(i)
    myproblem = problem(Vmax=25, Vmin=5, Umax=5, Umin=-5, Vc=15, L=300, S=50, safe_distance=20, delta_t=1)
    myproblem.setCAVs()
    SA = SimulatedAnnealing(myproblem, max_iter=100, init_temp=100, final_temp=1e-03, iter_per_temp=10, cooling_schedule="linear", beta=5)
    start_time = time.time()
    SA.perform_algorithm()
    end_time = time.time()
    best_values.append(SA.best_fvalue)
