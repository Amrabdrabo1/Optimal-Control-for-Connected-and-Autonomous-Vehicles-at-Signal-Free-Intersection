    ...
    def perform_algorithm(self):
        ...
        for iter in range(self.max_iter):
            ...
            for particle in self.particles:
                particle = particle.update_position(self.Nbest, self.w, self.c1, self.c2)
                if cost_func(particle.position) < best_fvalue:
                    best_particle = particle
                    best_fvalue = cost_func(particle.position)
...
for i in range(10):
    ...
    PSO.perform_algorithm()
    end_time = time.time()
    best_values.append(PSO.best_fvalue_in_each_iteration[-1])
