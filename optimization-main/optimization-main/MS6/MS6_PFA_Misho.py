...
class Particle:
    ...
    def update_position(self, pathFinder, old_pathfinder, neighbour, A, epsilon):
        not_feasible = True
        ...
        while not_feasible:
            ...
            for i in range(len(self.position)):
                for j in range(len(self.position[i].u) - 1):
                    ...
                    if j == len(self.position[i].u) - 2:
                        ...
                        if old_pathfinder == None:
                            old_u2 = 0
                        else:
                            old_u2 = old_pathfinder.position[i].u[j]
                        if cost_func(self.position) == cost_func(pathFinder.position):
                            while True:
                                if k < 50:
                                    r3 = random.uniform(0,1)
                                    self.position[i].u[j] = old_u + 2 * r3 * (old_u - old_u2) + A
                                else:
                            ...
                            while True:
                                if k < 50:
                                    ...
                                    R1 = alpha*r1
                                    R2 = beta*r2
                                    self.position[i].u[j] = old_u + R1 * (neighbour.position[i].u[j] - old_u) 
                                    + R2 * (pathFinder.position[i].u[j] - old_u)
                                    + abs(epsilon*(old_u-neighbour.position[i].u[j]))
                                else:
                                    self.position[i].u[j] = random.uniform(feasible_u[0], feasible_u[1])
                                k = k + 1
                                if self.position[i].is_velocity_valid(j + 1):
            ...
            iter += 1
            if iter == 100:
                self.position = generate_random_feasible_solution(create_cars())
                break
...
class PFA_Algorithm:
    ...
    def perform_algorithm(self):
        ...
        old_best_particle = None
        for iter in range(self.max_iter):
            print("iteration: ", iter)
            best_particle = self.pathFinder
            best_fvalue = cost_func(self.pathFinder.position)
            u1 = random.uniform(-1,1)
            u2 = random.uniform(-1,1)
            A = u2*math.exp(-2*iter/self.max_iter)
            epsilon = (1-iter/self.max_iter)*u1
            original = copy.deepcopy(self.particles)
            for i in range(len(self.particles)):
                while True:
                    r = random.randint(0,self.n_pop-1) 
                    neighbour = original[r]
                    if neighbour != self.particles[i] and neighbour != self.pathFinder :
                        break
                self.particles[i] = self.particles[i].update_position(self.pathFinder, old_best_particle, neighbour, A, epsilon)
            ...
            old_best_particle = self.pathFinder
            self.pathFinder = best_particle
