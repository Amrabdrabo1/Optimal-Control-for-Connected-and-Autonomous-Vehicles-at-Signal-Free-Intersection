...
class problem:
    def __init__(self, Vmax=25, Vmin=5, Umax=50, Umin=-50, Vc=15, L=300, S=50, safe_distance=20, delta_t=1):
        self.CAVs = []
        self.solution = []
    ...
    def obj_func_time(self, CAVs):  # to calculate total time
        ...
        for i in range(len(self.CAVs)):
            time = self.CAVs[i].Tf - self.CAVs[i].To
            total_time += time
        return total_time
    ...
    def obj_func_fuel(self, CAVs):  # To calculate total consumption fuel
        total_fuel = 0
        for i in range(len(self.CAVs)):
            for j in range(len(self.CAVs[i].u)):
                acc = self.CAVs[i].u[j][1]
                if(acc != -10):
                    total_fuel += acc**2
        return total_fuel
...
class SimulatedAnnealing:
        ...
        for iter in range(self.max_iter):
            for _ in range(self.iter_per_temp):
                ...
                if fvalue_neighbour < self.fvalues[-1]:
                    ...
                    self.time_array.append(self.problem.obj_func_time(sol_neighbour))
                    self.fuel_array.append(self.problem.obj_func_fuel(sol_neighbour))
                else:
                    ...
                    if np.random.rand() < p:
                        ...
                        self.time_array.append(self.problem.obj_func_time(self.sols[-1]))
                        self.fuel_array.append(self.problem.obj_func_fuel(self.sols[-1]))
    ...
    def visualize(self):
        # convergence plot
        plt.figure(1)
        plt.plot(np.arange(0, self.max_iter*self.iter_per_temp+1), self.fvalues, label = "Convergence Plot")
    ...
    def barChart(self):
        ...
        # set height of bar
        time = [self.time_array[0], self.time_array[99], self.time_array[199], self.time_array[299], self.time_array[399],
            self.time_array[499], self.time_array[599], self.time_array[699], self.time_array[799], self.time_array[899],self.time_array[999]]
        fuel = [(1/(self.problem.Umax*10))*self.fuel_array[0], (1/(self.problem.Umax*10))*self.fuel_array[99], (1/(self.problem.Umax*10))*self.fuel_array[199],
         (1/(self.problem.Umax*10))*self.fuel_array[299], (1/(self.problem.Umax*10))*self.fuel_array[399], (1/(self.problem.Umax*10))*self.fuel_array[499],
         (1/(self.problem.Umax*10))*self.fuel_array[599], (1/(self.problem.Umax*10))*self.fuel_array[699], (1/(self.problem.Umax*10))*self.fuel_array[799],
         (1/(self.problem.Umax*10))*self.fuel_array[899], (1/(self.problem.Umax*10))*self.fuel_array[999]]
...
myproblem = problem(Vmax=25, Vmin=5, Umax=50, Umin=-50, Vc=15,
